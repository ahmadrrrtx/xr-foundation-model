...
class BytePairEncoder(TokenizerInterface):
    ...
    # ------------------------------------------------------------------
    # Special-token contract (xrfm.tokenization.interface.Tokenizer)
    # ------------------------------------------------------------------

    @property
    def pad_token_id(self) -> int | None:
        """Id of ``<|pad|>`` if reserved, else ``None`` (never assume 0)."""
        return self.pad_id

    @property
    def bos_token_id(self) -> int | None:
        """Id of ``<|bos|>`` if reserved, else ``None``."""
        return self.bos_id

    @property
    def eos_token_id(self) -> int | None:
        """Id of ``<|endoftext|>``/``<|eos|>`` if reserved, else ``None``."""
        return self.eos_id

    @property
    def unk_token_id(self) -> int | None:
        """Id of ``<|unk|>`` if reserved, else ``None``."""
        return self.unk_id

    @classmethod
    def pretrained(cls) -> "BytePairEncoder":
        """Load the tokenizer packaged with XRFM (``tokenization/vocab.json``).

        Works regardless of the current working directory and from an
        installed wheel. Returns a tokenizer whose ``vocab_size()`` is the
        value training scripts must pass as the model's ``vocab_size``.
        """
        from importlib import resources

        tok = cls()
        vocab_resource = resources.files("xrfm.tokenization").joinpath("vocab.json")
        tok.load(str(vocab_resource))
        return tok
...
# Canonical short name (Phase 0). Same class; ``BytePairEncoder`` remains
# the historical name used across the codebase.
BPETokenizer = BytePairEncoder
