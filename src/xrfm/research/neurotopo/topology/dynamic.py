...
class DynamicTopology(nn.Module):
    ...
    def _build_candidate_edges(self, seed: int) -> None:
        ...
        self.register_buffer("edge_index", torch.tensor([targets, sources], dtype=torch.long), persistent=True)
        ...
        self.register_buffer("long_mask", ~self.is_local, persistent=False)
        self.register_buffer("local_mask", self.is_local, persistent=False)
    ...
    def score_longrange(self, H: torch.Tensor, M: torch.Tensor | None = None) -> torch.Tensor:
