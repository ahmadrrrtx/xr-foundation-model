...
from xrfm.research.neurotopo.model import NeuroTopoModel
...
def build_transformer(
    vocab: int, d_model: int, n_layers: int, n_heads: int = 4, max_seq_len: int = 256, pad_id: int = 0
) -> GPTModel:
...
def build_nt(
    vocab: int, d_model: int, n_layers: int, n_modules: int, module_dim: int, memory: bool = True, pad_id: int = 0
) -> NeuroTopoModel:
    cfg = NeuroTopoConfig(
        vocab_size=vocab,
        d_model=d_model,
        ...
        topology=TopologyConfig(
            n_modules=n_modules, module_dim=module_dim, local_degree=2, longrange_topk=4, rank=8, connectivity_reg=False
        ),
...
def match_nt_to_transformer(vocab: int, target_params: int, tolerance: float = 0.05) -> dict[str, Any]:
