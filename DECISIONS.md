...
## 2026-07-24 — v0.5.0 — Training Engine Design (Phase 5)
- **Problem:** Training loop must be original, config-driven, numerically stable, and support future distributed training (`FSDP`), `DeepSpeed`, `MoE`, `Multimodal`, `Reasoning` without rewrites.
- **Options:** (1) Raw Python loop (no abstraction), (2) `transformers` `Trainer` (external dependency), (3) `HuggingFace` `Accelerate` (external), (4) `DeepSpeed` `Engine` (complex, `RESEARCH-ONLY`), (5) Original `training_loop` with `ConfigLoader` integration, `optimizer` (`AdamW`), `scheduler` (`cosine` + `warmup`), `checkpoint` (`.pt` format + resume), `mixed_precision` (`bfloat16` + `GradScaler`), `gradient_clip` (configurable).
- **Chosen:** Original `training_loop` (`training/loop.py`) — design phase complete (`research/phase_05/RESEARCH.md`); implementation reserved after design review. `AdamW` (`optimizer`) = `CORE`. `Cosine` schedule (`scheduler`) = `CORE`. `Checkpoint` (`.pt` + resume) = `CORE`. `Mixed precision` (`bfloat16` + `GradScaler` hooks) = `CORE` / `OPTIONAL` (`True` by default; `False` for comparison). `Gradient clipping` (`torch.nn.utils.clip_grad_norm_`) = `CORE`.
- **Why:** Original loop ensures full control (`gradient_clip`, `mixed_precision`, `checkpoint` hooks are explicit, not hidden by external libraries). `AdamW` (`Kingma & Ba 2015`; `Loshchilov & Hutter 2019`) is standard modern practice (`Llama 3`, `DeepSeek-V3`, `Mistral 7B`). `Cosine` + `warmup` achieves best final performance (`Kaplan 2020`; `Chinchilla`). `.pt` checkpoint format is standard (`PyTorch` native; compatible with `FSDP` future extensions). `Mixed precision` (`bfloat16`) improves speed (`~1.5x`) and reduces memory (`~50%` for activations) without numerical instability (`GradScaler` handles underflow). `Gradient clipping` prevents explosion.
- **Reversible:** Later — `FSDP` (`PyTorch` native) can replace `optimizer.step()` with `FSDP` sharded step (`OPTIONAL`, `Phase 8`); `DeepSpeed` (`RESEARCH-ONLY`, future integration); `Gradient accumulation` (`OPTIONAL`, `Phase 8`); `Gradient checkpointing` (`RESEARCH-ONLY`, `Phase 9`); `Shampoo`/`Sophia` (`RESEARCH-ONLY`); `Constant`/`exponential` schedule (`RESEARCH-ONLY` comparison).
- **Classification:** `training_loop` (`original`) = `CORE`. `AdamW` (`optimizer`) = `CORE`. `Cosine` + `warmup` (`scheduler`) = `CORE`. `Checkpoint` (`.pt` + resume) = `CORE`. `Mixed precision` (`bfloat16` + `GradScaler`) = `CORE` (`True` by default) / `OPTIONAL` (`False` for `CPU` or comparison). `Gradient clipping` = `CORE`. `FSDP` hooks = `OPTIONAL` (`Phase 8`). `DeepSpeed`/`Megatron` = `RESEARCH-ONLY`. `Gradient accumulation` = `OPTIONAL` (`Phase 8`). `Shampoo`/`Sophia` = `RESEARCH-ONLY`. `Constant`/`exponential` schedule = `RESEARCH-ONLY`.

---

## 2026-07-24 — v0.5.0 — Numerical Stability Design (`Gradient` Safety, `Checkpoint` Integrity)
- **Problem:** Training must be numerically stable (no `NaN`/`Inf` in loss or gradients, no gradient explosion, no checkpoint corruption, no hidden numerical risks in `mixed_precision`).
- **Options:** (1) `Gradient clip` (`L2` norm) + `mixed_precision` (`GradScaler`) = standard modern practice; (2) No `gradient_clip` + `FP32` only = safer but slower; (3) `Adaptive gradient clipping` (`AGC` — `RESEARCH-ONLY` for very large models); (4) `Gradient checkpointing` (`torch.utils.checkpoint`) = memory efficiency but `RESEARCH-ONLY`.
- **Chosen:** `Gradient clip` (`torch.nn.utils.clip_grad_norm_`) = `CORE` (configurable `training.gradient_clip`, default `1.0`). `Mixed precision` (`torch.cuda.amp` + `GradScaler`) = `CORE` (`True` by default) / `OPTIONAL` (`False` for `CPU`). `Checkpoint` validation (`checksum` optional for integrity) = `RESEARCH-ONLY` (security-critical applications). `Gradient checkpointing` = `RESEARCH-ONLY` (`Phase 9`).
- **Why:** `Gradient clip` prevents explosion (`Kaplan 2020`; `Chinchilla` recommends `1.0` for `10M`-scale; `0.5` for larger). `Mixed precision` (`bfloat16`) improves speed and memory (`Dao 2024`; `Meta AI 2024`; `DeepSeek-AI 2024`) but requires `GradScaler` to prevent `underflow`. `Checkpoint` must include `ConfigLoader` settings for reproducibility (`DECISIONS.md` confirms this is `CORE`).
- **Reversible:** Later — `AGC` (`RESEARCH-ONLY`, `Phase 9`) can replace standard `clip_grad_norm_`; `Gradient checkpointing` (`RESEARCH-ONLY`) can be adopted for `XRFM-7B+`; `Checkpoint` format can be extended (`FSDP` state, `DeepSpeed` state — `OPTIONAL`, `Phase 8`).
- **Classification:** `Gradient clip` (`CORE`) = `CORE`. `Mixed precision` (`bfloat16` + `GradScaler`) = `CORE` (`True`) / `OPTIONAL` (`False`). `Checkpoint` integrity (`hash`/`checksum`) = `RESEARCH-ONLY`. `Gradient checkpointing` = `RESEARCH-ONLY` (`Phase 9`). `AGC` = `RESEARCH-ONLY` (`Phase 9`).
