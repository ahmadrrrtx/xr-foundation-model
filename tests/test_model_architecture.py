...
class TestGPTModel:
    ...
    def test_forward_shape_batch(self):
        model = GPTModel()
        model.eval()
        x = torch.randint(0, model.embedding.vocab_size, (2, 12))
        with torch.no_grad():
            logits, pkv = model(x)
        assert logits.shape == (2, 12, model.embedding.vocab_size)
        assert pkv is None  # use_cache=False

    def test_forward_single_seq(self):
        model = GPTModel()
        model.eval()
        x = torch.randint(0, model.embedding.vocab_size, (8,))
        with torch.no_grad():
            logits, pkv = model(x)
        assert logits.shape == (8, model.embedding.vocab_size)
        assert pkv is None

    def test_forward_with_cache(self):
        model = GPTModel()
        model.eval()
        x = torch.randint(0, model.embedding.vocab_size, (1, 8))
        with torch.no_grad():
            logits, pkv = model(x, use_cache=True)
        assert logits.shape == (1, 8, model.embedding.vocab_size)
        assert pkv is not None
        assert len(pkv) == len(model.blocks)  # one KV pair per layer
        for layer_kv in pkv:
            k, v = layer_kv
            assert k.shape[1] == model.blocks[0].attn.n_heads  # n_heads from model
    ...
    def test_cache_incremental(self):
        ...
        x1 = torch.randint(0, model.embedding.vocab_size, (1, 4))
        _, pkv = model(x1, use_cache=True)
        # Second pass: single new token with cache
        x2 = torch.randint(0, model.embedding.vocab_size, (1, 1))
        logits2, pkv2 = model(x2, past_key_values=pkv, use_cache=True)
        assert logits2.shape == (1, 1, model.embedding.vocab_size)
    ...
    def test_numerical_stability(self):
        model = GPTModel()
        model.eval()
        x = torch.randint(0, model.embedding.vocab_size, (2, 16))
        with torch.no_grad():
            logits, _ = model(x)
    ...
    def test_gradient_flow(self):
        model = GPTModel()
        model.train()
        x = torch.randint(0, model.embedding.vocab_size, (2, 6))
        logits, _ = model(x)
        loss = logits.sum()
        loss.backward()
    ...
    def test_vocab_bounds_error(self):
        ...
        with pytest.raises(IndexError, match="exceeds vocab"):
            model(torch.tensor([[model.embedding.vocab_size]]))
